...
//    println("Hello World!")
//    val derivationResult = DerivationPath.getJunctions1()
//
//    val seed = EthereumSeedFactory.deriveSeed(mnemonic, derivationResult.password).seed
...
fun ByteArray.ethereumAddressFromPublicKey(): ByteArray {
    ...
    return decompressed.keccak256().takeLast(20).toByteArray()//.copyLast(20)
}
...
//    val hmacResult = seed.hmacSHA512(secret = INITIAL_SEED)
    ...
    val privateKey = seed.sliceArray(0 until PRIVATE_KEY_SIZE)
    val chainCode = seed.sliceArray(PRIVATE_KEY_SIZE until seed.size)
